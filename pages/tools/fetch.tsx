...
const FetchTool: NextPage = () => {
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      const response = await fetch(`/api/tools/fetch?url=${url}&selector=${encodeURIComponent(selector)}`).then((res) => res.json())
  ...
  return (
    <>
      ...
      <section className="relative">
        ...
        {pageErrors ? <p className={'alert alert-danger py-4 rounded-lg mb-4'}>{pageErrors}</p> : null}

        <form onSubmit={handleSubmit(onSubmit)}>
          <div className={`input-control flex-shrink w-full ${errors.url?.type && 'input-control--errors'}`}>
            <label htmlFor="url">URL</label>
            <input id="url" placeholder="Your url" {...register('url')} />
          </div>

          <div className={`input-control flex-shrink w-full ${errors.selector?.type && 'input-control--errors'}`}>
            <label htmlFor="selector">Selector</label>
            <input id="selector" placeholder="Selector eg. #content" {...register('selector')} />
          </div>
          ...
          <button type="submit" className={`btn btn-primary mt-4 mb-8 ${loading ? 'is-loading' : ''}`} disabled={loading}>
            {loading ? (
            ...
            <div className="input-control">
              <label htmlFor="plainText" className="!flex items-center w-full">
                ...
                <div className="ml-auto">
                  ...
                  <div className={`input-control input-control--checkbox !mt-0 mr-8`}>
                    <input id="haveClasses" type="checkbox" checked={haveClasses} onChange={() => setHaveClasses(!haveClasses)} />

                    <label htmlFor="haveClasses">classes</label>
                  </div>

                  <div className={`input-control input-control--checkbox !mt-0`}>
                    <input id="isPlainText" type="checkbox" checked={isPlainText} onChange={() => setIsPlainText(!isPlainText)} />

                    <label htmlFor="isPlainText">Plain Text</label>
                  </div>
                </div>
              </label>
              <textarea name="plaintext" id="plaintext" placeholder="Result" rows={15} value={fetchResult} disabled />
              <span
                onClick={(e) => {
