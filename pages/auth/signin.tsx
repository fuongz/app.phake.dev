  ...
  return (
    <div className={'flex pt-44 justify-center items-center'}>
      <div className={'max-w-md mx-auto rounded-lg'}>
        ...
        {message ? <p className={'alert alert-success py-4 rounded-lg mb-4'}>{message}</p> : null}
        ...
        >
          {loading ? (
            ...
            <>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="mr-3 inline w-4 h-4 mb-0.5">
                <path
        ...
        {loginType === 'email' ? (
          <form onSubmit={handleSubmit(handleLogin)}>
            <div className="border-t border-gray-700 mt-6">
              <div className={`input-control ${errors.email?.type && 'input-control--errors'}`}>
                <label htmlFor="email">Email</label>
                <input id="email" type="email" placeholder="Your email address" {...register('email')} />
                {errors.email?.type && <p className="input-feedback">{errors.email?.message}</p>}
              </div>

              <div className={`input-control ${errors.password?.type && 'input-control--errors'}`}>
                ...
                <input
                  ...
                  {...register('password')}
                />
                {errors.password?.type && <p className="input-feedback">{errors.password?.message}</p>}
              </div>

              <div className="flex items-center mt-4">
                <button className={`btn btn-primary ${loading ? 'is-loading' : ''}`} type="submit" disabled={loading}>
                  {loading ? (
                    <>
                      <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
